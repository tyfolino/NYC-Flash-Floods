...
def load_and_merge(ffe_csv, alldays_csv):
    ...
    merged["node_ffe"] = merged.apply(
        lambda r: f"({int(r['node_ffe_i'])},{int(r['node_ffe_j'])})", axis=1
    )
    merged["node_all"] = merged.apply(
        lambda r: f"({int(r['node_all_i'])},{int(r['node_all_j'])})", axis=1
    )
...
def print_crosstab(merged, xffe, yffe, xall, yall):
    ...
    for bi in range(xall):
        for bj in range(yall):
            ...
            comp_str = ", ".join(
                f"{node}: {count} ({100 * count / n_events:.0f}%)"
...
def plot_heatmaps(merged, xffe, yffe, xall, yall, out_path):
    ...
    for i in range(xffe):
        for j in range(yffe):
            ...
            ax.set_title(f"FF-Only SOM Node {node_ffe}\nn={n_events}", fontsize=8)
...
def plot_flow(merged, xffe, yffe, xall, yall, out_path):
    ...
    for idx, (i, j) in enumerate(small_nodes):
        ...
        n = len(merged[merged["node_ffe"] == f"({i},{j})"])
        box = FancyBboxPatch(
    ...
    for idx, (i, j) in enumerate(big_nodes):
        ...
        n = len(merged[merged["node_all"] == f"({i},{j})"])
        box = FancyBboxPatch(
    ...
    for si, sj in small_nodes:
        ...
        for bi, bj in big_nodes:
            n_flow = len(subset[subset["node_all"] == f"({bi},{bj})"])
            if n_flow == 0:
                continue
            frac = n_flow / n_total
    ...
    for i in range(xall):
        for j in range(yall):
            ...
            for node_ffe in sorted(color_map.keys()):
                node_label = f"({node_ffe[0]},{node_ffe[1]})"
                count = len(subset[subset["node_ffe"] == node_label])
                if count > 0:
