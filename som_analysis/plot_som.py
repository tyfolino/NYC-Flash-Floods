...
def main():
    ...
    paths = get_paths(args.moisture_var, args.moisture_weight)
    ...
    moist_norm_weighted_ffe = load_moist_var(
        f"{SOM_INTERMEDIATE_PATH}{pfx}_norm_weighted_ffe.nc", var_name
    )
    ...
    z500_patterns_raw, _ = compute_composites(
        z500_ffe, bmus, xdim, ydim, time_dim="time"
    )
    ...
    for i in range(xdim):
        for j in range(ydim):
            ...
            min_lats, min_lons = [], []
            for k in idx:
                field = z500_norm_ffe.isel(time=k)
    ...
    node_labels_chi = [f"({i},{j})" for i in range(xdim) for j in range(ydim)]
