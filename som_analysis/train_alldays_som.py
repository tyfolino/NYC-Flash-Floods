...
from .helpers import load_moist_var
...
def main():
    ...
    som_days = pd.to_datetime(z500_norm_weighted_daily["time"].values).tz_localize(
        None
    )
    ...
    lat = z500_daily["lat"].values
    lon = z500_daily["lon"].values
    ...
    ff_timestamps = pd.to_datetime(
        z500_norm_weighted_daily["time"].values[event_indices]
    )
