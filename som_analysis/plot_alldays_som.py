...
def main():
    ...
    paths = get_alldays_paths(args.moisture_var, args.xdim, args.ydim)
    ...
    moist_norm_weighted_daily = load_moist_var(
        f"{SOM_INTERMEDIATE_PATH}{pfx}_norm_weighted_daily.nc", var_name
    )
    ...
    z500_flat = z500_norm_weighted_daily.stack(features=["lat", "lon"]).values
    ...
    for idx, (x, y) in enumerate(coords):
        ...
        plt.text(x, y, f"({ix},{iy})", fontsize=7, ha="center", va="center", zorder=5)
    ...
    z500_anom_comp, comp_counts = compute_composites(
        z500_norm_daily, bmus, xdim, ydim, time_dim="time"
    )
    ...
    z500_ff = z500_daily.isel(time=xr.DataArray(event_indices, dims="valid_time"))
    ...
    for other_var in other_vars:
        ...
        other_daily = load_moist_var(
            f"{SOM_INTERMEDIATE_PATH}{other_pfx}_daily.nc", other_var_name
        )
    ...
    for k in event_indices:
        ni, nj = bmus[k]
        if has_cutoff_low(z500_daily.isel(time=int(k)).values / 98.1):
    ...
    if not args.skip_indiv:
        ...
        z500_ff_indexed = z500_daily.isel(
            time=xr.DataArray(event_indices, dims="event")
        ).assign_coords(event=np.arange(n_ff))
        ...
        if "time" in z500_ff_indexed.coords:
            z500_ff_indexed = z500_ff_indexed.drop_vars("time")
